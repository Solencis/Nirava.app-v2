...
const MobileNavigation: React.FC = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-t border-stone/20 dark:border-gray-700 z-40 safe-area-inset-bottom transition-colors duration-300">
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center py-3 px-4 min-w-[60px] min-h-[60px] justify-center transition-all duration-300 rounded-xl relative ${
                active
                  ? 'text-wasabi dark:text-jade bg-wasabi/10 dark:bg-jade/20 scale-105 shadow-lg'
                  : 'text-stone dark:text-gray-400 hover:text-wasabi dark:hover:text-jade hover:bg-wasabi/5 dark:hover:bg-jade/10 active:scale-95'
              }`}
            >
              <Icon size={22} className="mb-1" />
              <span className="text-xs font-medium leading-tight">{item.label}</span>
              {active && (
                <div className="absolute -top-1 left-1/2 transform -translate-x-1/2 w-2 h-2 bg-wasabi dark:bg-jade rounded-full animate-pulse"></div>
              )}
            </Link>
          );
        })}
